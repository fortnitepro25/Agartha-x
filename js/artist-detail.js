// js/artist-detail.js  ←  FINAL WORKING VERSION (FI-tested)
const API_KEY = "2";
...
document.addEventListener("DOMContentLoaded", async () => {
  ...
  try {
    // PROXY FIX: corsproxy.io bypasses CORS + rate-limit blocks
    const searchUrl = `https://www.theaudiodb.com/api/v1/json/${API_KEY}/search.php?s=${encodeURIComponent(searchName)}`;
    ...
    document.getElementById("artist-thumb").src = artist.strArtistThumb || "../img/placeholder.jpg";
    document.getElementById("artist-bio").textContent =
      (artist.strBiographyEN || "No bio transmitted…").slice(0, 800) + "…";
    ...
    const albumRes = await fetch("https://corsproxy.io/?" + encodeURIComponent(albumUrl), {
      headers: { "User-Agent": "Agartha-X/1.0" }
    });
    ...
    if (albums.length === 0) {
      ...
      albums.forEach(alb => {
        ...
        card.innerHTML = `
          <img src="${alb.strAlbumThumb || '../img/noart.jpg'}" alt="${alb.strAlbum}">
          <p>${alb.strAlbum}<br><span class="year">${alb.intYearReleased || '????'}</span></p>
        `;
